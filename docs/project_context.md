...
- **관측 수단을 그리는 비용도 프레임 예산에 들어갑니다.** 값을 바꾸지 않아도
  프레임 시간을 늘리면 결과가 달라집니다. 고정 틱 루프에서는 프레임이 길어질수록
  밀린 틱이 늘고, `MaxTicksPerFrame` 상한에 닿으면 누산 시간을 버리기 때문입니다.
  실제로 커스텀 인스펙터가 매 프레임 `AssetDatabase.FindAssets` 를 호출해 프레임을
  66ms 이상으로 늘린 사례가 있습니다. `CustomEditor` 는 그리는 빈도를 제한하고
  변하지 않는 조회는 `OnEnable` 에서 한 번만 해야 합니다.
  근거는 `docs/notes/perf-01-inspector-repaint-cost.md` 참조
...
- **네트워크 지연 조건은 클라이언트 피어에만 걸립니다** (2026-08-28 추가). MPPM 가상
  ...
  역할을 확인해 클라이언트에서만 프리셋을 적용합니다. Host 화면에서 캐릭터가 즉시
  반응하는 것은 정상이며, 리슨 서버라 자기 입력이 네트워크를 타지 않기 때문입니다.
- **`NetworkSimulator` 컴포넌트는 런타임에 붙습니다** (2026-08-28 추가). 씬에 미리
  넣지 않고 `NetworkConditionSimulator.Attach` 가 `DevHud` 에 추가합니다. 직렬화할
  값이 없어 씬 배선을 늘릴 이유가 없기 때문이며, Play 중에는 인스펙터에 그대로
  보이므로 손으로 값을 바꿔 볼 수도 있습니다. 씬에 이미 하나 있으면 그것을 씁니다.
- **시뮬레이터 스테이지는 `UNITY_MP_TOOLS_NETSIM_IMPLEMENTATION_ENABLED` 가 정의된
  경우에만 파이프라인에 들어갑니다** (2026-08-28 확인). 이 define 은 NGO 의
  `UnityTransport` 안에서만 쓰이고 에디터에서는 자동으로 정의됩니다. 정의되지 않은
  채로 돌면 프리셋을 넣어도 조용히 아무 일도 일어나지 않습니다. 그래서 HUD 표시는
  "요청한 조건"일 뿐이고, 실제로 걸렸는지는 반드시 RNSM 의 RTT 로 확인합니다.
...
| 촬영용 지연 조건 | **단방향 75ms / 지터 0 / 손실 3%** | 2026-08-28 확정. 클라이언트 피어에만 건다. 시뮬레이터 스테이지가 송신과 수신 양쪽에 걸리므로 이 피어가 겪는 RTT 는 150ms 다. 지터를 0 으로 둔 것은 RTT 가 흔들리면 "150ms 조건에서 찍었다"는 근거가 약해지기 때문이다. 값은 `Game/NetworkConditionController` 의 상수 |
| 보간 지연 버퍼 | 미정 | 100~200ms 범위 |
| 입력 버퍼 크기 | 미정 | 60Hz 기준 최소 15틱. 마스킹 위해 2의 거듭제곱 권장 |
| 재조정 허용 오차 | 미정 | 위치 임계값 |
| 히트박스 히스토리 보관 시간 | 미정 | 랙 보상용. 300ms 면 18틱 |
| 기준선 대역폭 | 미정 | 최적화 전 bytes/sec. 정지 상태와 이동 상태를 나눠 측정한다. 정지 상태에도 입력 RPC 가 초당 60개 나가므로 두 값의 차이가 작을 것으로 예상되며, 그 예상이 맞는지 자체가 나이브 구현의 낭비를 보여주는 근거다. 측정은 Network Profiler 로 하고 지연 조건을 끈 상태에서 한다 |
