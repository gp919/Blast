...
포트폴리오용 2인 코옵 2D 플랫포머 전투 게임입니다. Netcode for GameObjects 를 기반으로 합니다.
...
C++/DirectX 9, 11로 자체 엔진을 여러 개 만든 경험을 보유하고 있습니다. 렌더링 파이프라인, 물리 충돌,
HFSM, 에디터 툴을 직접 구현했습니다. Unity와 C#은 이번이 첫 프로젝트입니다.
...
면접에서 네트코드는 깊게 파고드는 주제이므로 한 줄씩 근거를 제시할 수 있어야 합니다.
...
작업자가 직접 작성한 코드의 리뷰는 적극적으로 수행하세요. 특히 되감기 시작 틱 오프셋, 버퍼 인덱싱,
결정성 위반, 매 프레임 GC 할당을 중점적으로 봅니다.

### 바로 진행해도 되는 영역

에디터 확장, 디버그 오버레이, 개발 툴, 스냅샷 직렬화 보일러플레이트, 링버퍼 등 자료구조,
테스트 하네스, 결정성 검증 스크립트, 리팩터링, 문서화 작업은 확인 없이 바로 진행해도 됩니다.
...
기능을 구현할 때 어느 수단을 사용할지 함께 제안하세요. 관측 수단이 없으면 완료로
보고하지 마세요.

**이슈 하나의 동작 확인이 끝나면 `docs/feature-closeout.md` 의 마무리 절차를
수행하세요.** 코드 리뷰, Unity 개념 대응 정리, 문서 갱신, 다음 이슈 초안까지가
한 덩어리입니다. 작업자가 먼저 요청하지 않아도 절차를 시작하겠다고 알리세요.
챗을 언제 마무리할지도 그 문서에 정리되어 있습니다.
...
직접 수정하면 프로젝트가 손상되는 파일들입니다.

- `*.meta` 직접 편집 및 삭제 금지. GUID 참조가 깨집니다
- `*.prefab`, `*.unity`, `*.asset` YAML 수동 편집 금지
- `Library/`, `Temp/`, `obj/`, `Logs/`, `UserSettings/` 는 생성물입니다. 읽거나 쓰지 마세요
...
Input과 Simulation을 동시에 참조해야 하는 코드(틱 드라이버 등)는 전부 여기에 둡니다.
...
코드를 수정한 뒤에는 이 스크립트부터 실행하세요.

멀티플레이 테스트는 MPPM(Multiplayer Play Mode) 가상 플레이어를 사용합니다. 빌드는 필요하지 않습니다.
...
항목 추가를 제안하세요. 설계 판단 오류, 네트코드 로직 오류, 결정성 위반, 성능 함정,
그리고 기존 구조를 확인하지 않아 생긴 중복이나 충돌만 기록합니다.
오타나 단순 컴파일 에러는 제외합니다.
